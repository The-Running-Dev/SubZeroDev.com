...
`assertImportGraph`'s alone, and that function is declared with no implementation — see
[`V16`](#invariants). The union above is therefore the **specification** the missing implementation
must satisfy, not a transcription of what the module declares today; the three rows below describe
what each code will mean when something raises it. **Stated plainly so it is a limit rather than an
assumption:** the three import rules they carry are checked today by
`tests/content/import-graph.test.ts` against a test-local AST helper, which raises none of these
codes, so a reader must not take their presence here as evidence any of them can fire. The same
standing `V13`'s per-document counts have, recorded here for the same reason — a rule believed
enforced is worse than one known not to be.
...
| `ServedBytesMismatch` | What a target serves for `/`, `/cv/` or `/portfolio/` differs byte for byte from that route's own emitted document. **Both targets raise it** — the running image at the in-CI gate and Pages at its read-back — which is what `V11` requires and what makes the pair comparable | No | From the image gate: fail it and **never push**. From the Pages read-back: fail it and **announce no preview URL**. The two callers differ because the gates sit on different sides of publication, and a row naming only the first read as though the preview had no byte check |
...
| **V2** | Loading a route document triggers zero requests other than the navigation document itself. Checked against **all four** emitted documents: `/`, `/cv/` and `/portfolio/`, each a directly navigable route reached from the masthead, and the **miss document**, reached the only way it is reachable — through the unknown-path fallback `V12` covers, which resolves to root `404.html` at a 404 status. The miss half was written as *excluded* from 2026-08-21 until 2026-08-24 and was false the whole time; the capture had covered it since `S8`, so the clause licensed deleting a real check as out of scope. What the miss document costs to cover is nothing — it carries no script element at all (`X10`), so the assertion is cheap and the exclusion bought only the sentence | Verification |
