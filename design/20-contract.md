...
| `ScriptElementPresent` | An emitted document contains a script element that is not the single permitted `application/ld+json` block — a missing or different `type`, a `src` attribute, or a second such block (`V13`, `X6`) | No | Fail the build |
